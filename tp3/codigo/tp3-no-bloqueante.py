...
class Node(object):
		...
		while len(queue) > 0 or cant_enviados > 0:
			...
			while len(queue) > 0:
				...
				if node_rank not in processed:
					...
					self.__comm.isend(thing_hash, dest=node_rank, tag=TAG_NODE_FIND_NODES_REQ)
					cant_enviados += 1

			res = self.__comm.recv(source=MPI.ANY_SOURCE, tag=TAG_NODE_FIND_NODES_RESP)
			cant_enviados -= 1
			# res = [hash, rank]
	...
	# con tag store envio los archivos al nodo (en un mensaje aparte), no espero nada.
		...
		while len(queue) > 0 or cant_enviados > 0:
			...
			for fhash, fpath in files.iteritems():
				self.__files[fhash] = fpath

		# Paso los mas cercanos a nodes_min y lo devuelvo
		for nhash, nrank in mas_cercanos.items():
			nodes_min.add((nhash, nrank))
	...
	def __handle_node_look_up(self, source, data):
		...
		# Esto tira un exception si no esta!
		# data = self.__files[file_hash]
		# Devuelvo un booleano si no está
		if file_hash in self.__files:
			data = self.__files[file_hash]
		else:
			data = False
